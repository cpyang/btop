...
namespace Tools {
	...
	constexpr auto ssplit(std::string_view str, char delim = ' ') {
                auto _r = str | std::views::split(delim) | std::views::filter([](auto&& range) { return !std::ranges::empty(range); }); std::vector<std::string> _v; for(auto&& _p : _r) _v.emplace_back(_p.begin(), _p.end()); return _v;
	}
